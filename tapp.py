...
def main():
    ...
    parser.add_argument('search_area', action='store', type=str, 
        help='Text file containing the northwestern and southwestern '
        'coordinates of the search area in decimal degrees, comma separated')
    ...
    parser.add_argument('camera_params', action='store', type=str, 
        help='Text file containing the tagged camera parameters')
    ...
    retval = gdal_utils.trim(target_area[0][1], target_area[0][0], 
        target_area[1][1], target_area[1][0], geotiff_file, '.tmp.tiff')
    ...
    poses = np.loadtxt(poses_file, delimiter=",", 
        dtype={ 'names': ('LAT', 'LON', 'ALT', 'ROLL', 'PITCH', 'YAW'), 
                'formats': ('f8', 'f8', 'f8', 'f8', 'f8', 'f8')}, skiprows=1)
    ...
    for pose in poses:
        ...
        pose['LAT'] = x
        pose['LON'] = y
        print("%f, %f, %f, %f, %f, %f" % (pose['LAT'], pose['LON'], 
            pose['ALT'], pose['ROLL'], pose['PITCH'], pose['YAW']))
        camera.snap(pose)
